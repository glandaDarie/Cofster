...
from utils.paths.cappuccino_attrs_path import CAPPUCCINO_ATTRS_PATH
...
class CappuccinoCreator(CoffeeCreator):
    ...
    @initialize_drink(*drinks_pin_mapper[CoffeeTypes.CAPPUCCINO.value], type_input=GPIO.OUT)
    def do(self, cappuccino_data : Dict[str, Any], *, pins : List[int] | None = None) -> Tuple[str]:
        params : Dict[str, Any] = {
            "drink_data" : cappuccino_data,
            "pins" : pins, 
            "parallel_coffee_creator_service" : self.parallel_coffee_creator_service,
            "attrs_path" : CAPPUCCINO_ATTRS_PATH
        }
