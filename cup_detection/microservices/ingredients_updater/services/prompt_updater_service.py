...
class PromptUpdaterService:
    ...
    def __call__(self, customer_name : str, prompt_recipe : str, limit_nr_responses : int = 10) -> str | None:
        try:
            person_responses = self.questionnaire_database_service.get_customer_responses(customer_name=customer_name, \
            ...
            new_user_file_prompt = self.openai_service(prompt=prompt_recipe, chat_history=chat_history)
            ...
            error_msg : str = f"Table {self.database_handler.table_name} could not be found in the database. Error: {table_error}"
            return error_msg
