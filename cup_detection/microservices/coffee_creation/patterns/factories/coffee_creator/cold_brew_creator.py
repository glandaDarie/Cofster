...
from utils.paths.cold_brew_attrs_path import COLD_BREW_ATTRS_PATH
...
class ColdBrewCreator(CoffeeCreator):
    ...
    @initialize_drink(*drinks_pin_mapper[CoffeeTypes.COLD_BREW.value], type_input=GPIO.OUT)
    def do(self, cold_brew_data : Dict[str, Any], *, pins : List[int] | None = None) -> Tuple[str]:
        params : Dict[str, Any] = {
            "drink_data" : cold_brew_data, 
            "pins" : pins, 
            "parallel_coffee_creator_service" : self.parallel_coffee_creator_service,
            "attrs_path" : COLD_BREW_ATTRS_PATH
        }
